...
from agent.embedding_cache import get_cached_embeddings, get_cached_api_spec
import sys
...
class State:
    ...
    relevant_endpoints: Optional[List[Dict[str, Any]]] = None
...
async def extract_api_spec(state: State, config: RunnableConfig) -> Dict[str, Any]:
    ...
    if cache_file:
        print(f"🔍 Checking for cached api spec at {cache_file}...")
        cached_spec = await get_cached_api_spec(api_url, cache_file)
        if cached_spec:
            print(f"✅ Using cached API spec for {api_url}")
            return {"api_spec": cached_spec}

    # Fallback to downloading
    print(f"📥 Downloading API spec from {api_url}")
...
async def rag_retrieve_endpoints(state: State, config: RunnableConfig) -> Dict[str, Any]:
    ...
    if cache_file:
        print(f"🔍 Checking for cached embeddings at {cache_file}...")
        cached_data = await get_cached_embeddings(api_url, cache_file)

        if cached_data:
            print(f"✅ Using cached embeddings for {api_url}")
            endpoint_documents = cached_data["endpoint_documents"]
            doc_embds = cached_data["embeddings"]
        else:
            print(f"⚠️ No cached embeddings found, creating new ones...")
...
async def find_relevant_endpoints(state: State, config: RunnableConfig) -> Dict[str, Any]:
    ...
    llm = ChatAnthropic(
        ...
        max_tokens=Config.FIND_ENDPOINTS_MAX_TOKENS  # type: ignore
    )
...
async def construct_http_request(state: State, config: RunnableConfig) -> Dict[str, Any]:
    ...
    llm = ChatAnthropic(
        model=Config.MODEL_NAME,  # type: ignore
        max_tokens=Config.CONSTRUCT_REQUEST_MAX_TOKENS  # type: ignore
    )
    ...
    if "schemas" in components:
        ...
        limited_components["schemas"] = {k: components["schemas"][k] for k in schema_keys}
